...
function RunPayrollModal({ onClose, onConfirm }) {
  ...
  const totalNet   = eligible.reduce((s,e)=>s+net(e),0);
...
// ── EDIT SALARY MODAL ─────────────────────────────────────────────────────────
const EMP_TYPE_INFO = {
  Regular:     { color:"#5af07a", bg:"#0f1f0f", desc:"Full statutory deductions apply (SSS, PhilHealth, Pag-IBIG, withholding tax)." },
  Contractual: { color:"#f0c85a", bg:"#1f1a0f", desc:"No statutory deductions. Gross = Net (flat agreed rate). No benefits." },
  "Part-time":   { color:"#5a9af0", bg:"#0a1a2a", desc:"Statutory contributions prorated at 50%. Benefits included." },
  Custom:      { color:"#aaaaaa", bg:"#1a1a1a", desc:"Per-deduction overrides configured below. Applies only to statutory items." },
};
const TAX_EXEMPT_OPTIONS = ["Minimum Wage Earner","DOLE Exemption","Tax Treaty Exempt","PWD / Senior Citizen","Other"];

function EditSalaryModal({ emp, onClose, onSave, settings }) {
  const cfg = settings || DEFAULT_SETTINGS;
  const [salary,   setSalary]   = useState(emp.salary);
  const [freq,     setFreq]     = useState(emp.payFreq);
  const [tax,      setTax]      = useState(emp.tax);
  const [benefits, setBenefits] = useState(emp.benefits);
  const [empType,  setEmpType]  = useState(emp.empType || "Regular");
  const [taxExempt,setTaxExempt]= useState(emp.taxExempt || false);
  const [exemptReason, setExemptReason] = useState(typeof emp.taxExempt==="string"?emp.taxExempt:"");
  const [overrides,setOverrides]= useState(emp.deductOverrides || {sss:true,philhealth:true,pagibig:true});
  const [reason,   setReason]   = useState("");
  const [tab, setTab]           = useState("comp"); // "comp" | "class"

  const previewEmp = { ...emp, salary, payFreq:freq, tax, benefits, empType, taxExempt: taxExempt?(exemptReason||true):false, deductOverrides:overrides };
  const d = calcDeductions(previewEmp, cfg);

  const typeInfo = EMP_TYPE_INFO[empType] || EMP_TYPE_INFO.Regular;
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
        <div className="w-full max-w-lg rounded-xl flex flex-col" style={{backgroundColor:"#0d0d0d",border:"1px solid #2a2a2a",maxHeight:"92vh"}}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  <p className="text-gray-500 text-xs" style={{fontFamily:"system-ui,sans-serif"}}>{emp.role}</p>
                  <span className="text-xs px-1.5 py-0.5 rounded" style={{fontFamily:"system-ui,sans-serif",backgroundColor:typeInfo.bg,color:typeInfo.color}}>{empType}</span>
                  {taxExempt && <span className="text-xs px-1.5 py-0.5 rounded" style={{fontFamily:"system-ui,sans-serif",backgroundColor:"#1a0a1a",color:"#c07af0"}}>Tax Exempt</span>}
                </div>
              </div>
            </div>
          ...
          <div className="flex border-b flex-shrink-0" style={{borderColor:"#1a1a1a"}}>
            {[["comp","Compensation"],["class","Classification & Exemptions"]].map(([key,label])=>(
              <button key={key} onClick={()=>setTab(key)} className="px-5 py-2.5 text-sm transition-all"
                style={{fontFamily:"system-ui,sans-serif",color:tab===key?"#fff":"#555",borderBottom:tab===key?"2px solid #fff":"2px solid transparent"}}>
                {label}
              </button>
            ))}
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5">
            ...
            {tab==="comp" && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs uppercase tracking-widest text-gray-500 mb-1.5" style={{fontFamily:"system-ui,sans-serif"}}>Annual Salary</label>
                    <div className="relative"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 text-sm">$</span><input type="number" className={IC} style={{...IS,paddingLeft:"1.75rem"}} value={salary} onChange={e=>setSalary(Number(e.target.value))}/></div>
                  </div>
                  <div>
                    <label className="block text-xs uppercase tracking-widest text-gray-500 mb-1.5" style={{fontFamily:"system-ui,sans-serif"}}>Pay Frequency</label>
                    <select className={IC} style={IS} value={freq} onChange={e=>setFreq(e.target.value)}>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs uppercase tracking-widest text-gray-500 mb-1.5" style={{fontFamily:"system-ui,sans-serif"}}>
                      Tax Rate (%) {taxExempt&&<span style={{color:"#c07af0"}}>· Exempt</span>}
                    </label>
                    <input type="number" className={IC} style={{...IS,opacity:taxExempt?0.4:1}} disabled={!!taxExempt} value={tax} onChange={e=>setTax(Number(e.target.value))}/>
                    {taxExempt&&<p className="text-xs mt-1" style={{fontFamily:"system-ui,sans-serif",color:"#c07af0"}}>Overridden — employee is tax exempt.</p>}
                  </div>
                  <div>
                    <label className="block text-xs uppercase tracking-widest text-gray-500 mb-1.5" style={{fontFamily:"system-ui,sans-serif"}}>
                      Benefits / yr ($) {empType==="Contractual"&&<span style={{color:"#f0c85a"}}>· N/A</span>}
                    </label>
                    <input type="number" className={IC} style={{...IS,opacity:empType==="Contractual"?0.4:1}} disabled={empType==="Contractual"} value={benefits} onChange={e=>setBenefits(Number(e.target.value))}/>
                  </div>
                </div>
                <div>
                  <label className="block text-xs uppercase tracking-widest text-gray-500 mb-1.5" style={{fontFamily:"system-ui,sans-serif"}}>Reason for Change</label>
                  <textarea className={IC} style={{...IS,height:64,resize:"none"}} placeholder="Annual review, promotion, market adjustment…" value={reason} onChange={e=>setReason(e.target.value)}/>
                </div>

                {/* Live deduction breakdown */}
                <div className="rounded-lg overflow-hidden" style={{border:"1px solid #1e1e1e"}}>
                  <div className="px-4 py-2.5" style={{backgroundColor:"#0a0a0a",borderBottom:"1px solid #1e1e1e"}}>
                    <p className="text-xs uppercase tracking-widest text-gray-600" style={{fontFamily:"system-ui,sans-serif"}}>Live Pay Preview · Per Period</p>
                  </div>
                  {[
                    ["Gross Pay",        fmt(d.g),           "#fff",    false],
                    ["Withholding Tax",  taxExempt?"Exempt":`-${fmt(d.taxAmt)}`, taxExempt?"#c07af0":"#f0c85a", false],
                    ["SSS",             empType==="Contractual"?"Exempt":`-${fmt(d.sssAmt)}`,  empType==="Contractual"?"#f0c85a":"#f0c85a", false],
                    ["PhilHealth",      empType==="Contractual"?"Exempt":`-${fmt(d.philAmt)}`, empType==="Contractual"?"#f0c85a":"#f0c85a", false],
                    ["Pag-IBIG",        empType==="Contractual"?"Exempt":`-${fmt(d.pagAmt)}`,  empType==="Contractual"?"#f0c85a":"#f0c85a", false],
                    ["Benefits",        empType==="Contractual"?"Exempt":`-${fmt(d.benAmt)}`,  "#888", false],
                    ["Net Pay",         fmt(d.net),          "#5af07a", true],
                  ].map(([l,v,c,bold],i,arr)=>(
                    <div key={l} className="flex justify-between px-4 py-2" style={{borderBottom:i<arr.length-1?"1px solid #141414":"none",backgroundColor:bold?"#111":"#0d0d0d"}}>
                      <span className={`text-sm ${bold?"text-white":"text-gray-500"}`} style={{fontFamily:"system-ui,sans-serif"}}>{l}</span>
                      <span className="text-sm" style={{fontFamily:"monospace",color:c,fontWeight:bold?600:400}}>{v}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {tab==="class" && (
              <div className="space-y-5">
                ...
                {/* Employment type */}
                <div>
                  <label className="block text-xs uppercase tracking-widest text-gray-500 mb-3" style={{fontFamily:"system-ui,sans-serif"}}>Employment Type</label>
                  <div className="grid grid-cols-2 gap-2">
                    {Object.entries(EMP_TYPE_INFO).map(([type, info])=>(
                      <button key={type} onClick={()=>setEmpType(type)}
                        className="px-4 py-3 rounded-lg text-left transition-all"
                        style={{fontFamily:"system-ui,sans-serif",backgroundColor:empType===type?info.bg:"#111",border:empType===type?`1px solid ${info.color}22`:"1px solid #2a2a2a"}}>
                        <p className="text-sm font-medium mb-0.5" style={{color:empType===type?info.color:"#888"}}>{type}</p>
                        <p className="text-xs leading-relaxed" style={{color:"#555"}}>{info.desc}</p>
                      </button>
                    ))}
                  </div>
                </div>

                {/* Custom overrides — only shown for Custom type */}
                {empType==="Custom" && (
                  <div className="rounded-lg p-4 space-y-3" style={{backgroundColor:"#111",border:"1px solid #2a2a2a"}}>
                    <p className="text-xs uppercase tracking-widest text-gray-500 mb-2" style={{fontFamily:"system-ui,sans-serif"}}>Statutory Deduction Overrides</p>
                    {[["sss","SSS / Social Security"],["philhealth","PhilHealth"],["pagibig","Pag-IBIG"]].map(([key,label])=>(
                      <div key={key} className="flex items-center justify-between">
                        <span className="text-gray-300 text-sm" style={{fontFamily:"system-ui,sans-serif"}}>{label}</span>
                        <button onClick={()=>setOverrides(o=>({...o,[key]:!o[key]}))}
                          className="w-10 h-5 rounded-full relative transition-all"
                          style={{backgroundColor:overrides[key]?"#fff":"#2a2a2a"}}>
                          <div className="w-3.5 h-3.5 rounded-full absolute top-0.5 transition-all"
                            style={{backgroundColor:overrides[key]?"#000":"#666",left:overrides[key]?"calc(100% - 18px)":"2px"}}/>
                        </button>
                      </div>
                    ))}
                    <p className="text-xs text-gray-600" style={{fontFamily:"system-ui,sans-serif"}}>These override the global statutory settings only for this employee.</p>
                  </div>
                )}
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <label className="text-xs uppercase tracking-widest text-gray-500" style={{fontFamily:"system-ui,sans-serif"}}>Tax Exemption</label>
                    <button onClick={()=>{ setTaxExempt(t=>!t); if(taxExempt) setExemptReason(""); }}
                      className="w-10 h-5 rounded-full relative transition-all"
                      style={{backgroundColor:taxExempt?"#c07af0":"#2a2a2a"}}>
                      <div className="w-3.5 h-3.5 rounded-full absolute top-0.5 transition-all"
                        style={{backgroundColor:taxExempt?"#000":"#666",left:taxExempt?"calc(100% - 18px)":"2px"}}/>
                    </button>
                  </div>

                  {taxExempt && (
                    <div className="space-y-2">
                      <p className="text-xs text-gray-500 mb-2" style={{fontFamily:"system-ui,sans-serif"}}>Select exemption reason (required for payroll records)</p>
                      <div className="grid grid-cols-1 gap-1.5">
                        {TAX_EXEMPT_OPTIONS.map(opt=>(
                          <button key={opt} onClick={()=>setExemptReason(opt)}
                            className="px-3 py-2.5 rounded text-sm text-left transition-all"
                            style={{fontFamily:"system-ui,sans-serif",backgroundColor:exemptReason===opt?"#1a0a1a":"#111",color:exemptReason===opt?"#c07af0":"#666",border:exemptReason===opt?"1px solid #4a1a6a":"1px solid #2a2a2a"}}>
                            {opt}
                          </button>
                        ))}
                      </div>
                      {!exemptReason && <p className="text-xs" style={{fontFamily:"system-ui,sans-serif",color:"#f05a5a"}}>⚠ Exemption reason required before saving.</p>}
                    </div>
                  )}

                  {!taxExempt && (
                    <div className="rounded-lg px-4 py-3" style={{backgroundColor:"#111",border:"1px solid #1e1e1e"}}>
                      <p className="text-xs text-gray-600" style={{fontFamily:"system-ui,sans-serif"}}>Employee is subject to standard withholding tax based on their configured rate.</p>
                    </div>
                  )}
                </div>
          ...
          <div className="px-6 py-4 flex items-center justify-between flex-shrink-0" style={{borderTop:"1px solid #1e1e1e"}}>
            ...
            <button
              disabled={taxExempt && !exemptReason}
              onClick={()=>{ onSave({...emp,salary,payFreq:freq,tax,benefits,empType,taxExempt:taxExempt?(exemptReason||true):false,deductOverrides:overrides}); onClose(); }}
              className="px-5 py-2 rounded text-sm font-medium hover:opacity-80 transition-all"
              style={{fontFamily:"system-ui,sans-serif",backgroundColor:taxExempt&&!exemptReason?"#1a1a1a":"#fff",color:taxExempt&&!exemptReason?"#444":"#000",cursor:taxExempt&&!exemptReason?"not-allowed":"pointer"}}>
              Save Changes ✓
            </button>
          </div>
        </div>
...
export default function PayrollPage() {
  ...
  return(
    <div className="min-h-screen text-white flex flex-col" style={{fontFamily:"'Georgia',serif",backgroundColor:"#000"}}>
          ...
          {activeTab==="employees"&&(
            <div className="space-y-5">
              ...
              <div className="rounded-lg overflow-hidden" style={{border:"1px solid #1e1e1e"}}>
                <table className="w-full text-sm">
                  <thead><tr style={{backgroundColor:"#0a0a0a",borderBottom:"1px solid #1e1e1e"}}>
                    {["Employee","Type","Department","Annual Salary","Pay Freq.","Per Period (Gross)","Tax Rate","Net/Period","Status",""].map(h=>(
                        ...
                        <tr key={emp.id} className="group transition-colors"
                          ...
                          <td className="px-4 py-3">
                            <div className="flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                              ...
                              <button onClick={()=>setEditEmp(emp)} className="text-xs px-2 py-1 rounded hover:opacity-80" style={{backgroundColor:"#111",color:"#aaa",border:"1px solid #2a2a2a",fontFamily:"system-ui,sans-serif"}}>Edit</button>
                            </div>
                          </td>
                        </tr>
                      );
      ...
      {editEmp&&<EditSalaryModal emp={editEmp} onClose={()=>setEditEmp(null)} onSave={handleSave} settings={settings}/>}
